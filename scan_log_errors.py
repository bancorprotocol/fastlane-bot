...
def find_latest_log_with_string(logs_dir, search_string, non_search_patterns, time_format="%Y%m%d-%H%M%S"):
    latest_time = None
    latest_folder = None
    ...
    for folder_name in os.listdir(logs_dir):
        ...
        if os.path.isdir(folder_path) and is_valid_timestamp_dir(folder_name, time_format):
            ...
            if os.path.exists(log_file_path):
                is_match = contains_only_one_specific_word(log_file_path, search_string, non_search_patterns)
                if is_match:
...
def scan_logs_for_opportunity(logs, search_pattern, max_minutes, latest_opportunity_timestamp=None):
    ...
    if error_detected:
        raise ValueError(f"Opportunity not found within {max_minutes} minutes interval.")
...
def main(logs_directory, interval, search_pattern, max_minutes):
    ...
    latest_folder = find_latest_log_with_string(logs_directory, search_pattern, non_search_patterns)
    ...
    while True:
        ...
        try:
            ...
            scan_logs_for_opportunity(logs, r"Opportunity with profit:", max_minutes, last_timestamp)
            print("Everything is OK...")
        except ValueError as e:
            next_latest_folder = find_latest_log_with_string(logs_directory, search_pattern, non_search_patterns)
