...
class Univ3Calculator:
    ...
    class DecimalsMissingError(Exception):
        pass
    ...
    def __post_init__(self, tkn0decv=None, tkn1decv=None, addrdec=None):
        ...
        if tkn0decv is None:
            try:
                ...
                raise self.DecimalsMissingError(
                    f"must provide tkn0decv for {self.tkn0}"
                )
        else:
        ...
        if tkn1decv is None:
            try:
                ...
                raise self.DecimalsMissingError(
                    f"must provide tkn1decv for {self.tkn1}"
                )
        else:
    ...
    def L(self):
        ...
        return (
            self.liquidity / 10 ** (0.5 * (self.tkn0dec + self.tkn1dec))
            if self.liquidity != 0
            else 0
        )
    ...
    def tkn0reserve(self, *, incltoken=False):
        ...
        sqrtP = sqrt(self.p)
        tkn0reserve = (
            self.liquidity * (sqrtPb - sqrtP) / (sqrtP * sqrtPb)
            if (sqrtP * sqrtPb) != 0
            else 0
        )
    ...
    def cpc_params(self, **kwargs):
        ...
        if pmar < 0:
            ...
            assert (
                pmar > -1e-10
            ), f"pm below pa beyond rounding error [{pm}, {pa}, {pmar}]"
        if abs(pmar) < 1e-10:
        ...
        if pmbr < 0:
            ...
            assert (
                pmbr < 1e-10
            ), f"pm abve pb beyond rounding error [{pm}, {pb}, {pmbr}]"
        if abs(pmbr) < 1e-10:
