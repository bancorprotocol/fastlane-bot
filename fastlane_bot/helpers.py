...
class SearchHelpers(BaseHelper):
    ...
    def generate_carbon_tvl_dict(
        ...
        for pair in pairs:
            ...
            if not check_if_tkn_in_table_address(
                pair[0]
            ) or not check_if_tkn_in_table_address(pair[1]):
                unknown_pairs.append(pair_combined)
                continue

            tokens.append(pair[0])
            tokens.append(pair[1])
            pair_dict[pair_combined] = {
                get_token_symbol_from_address(pair[0]): 0,
                get_token_symbol_from_address(pair[1]): 0,
                "total": 0,
            }
        ...
        for token in unique_tokens:
            price_list[token.lower()] = self.request_coingecko_price(
                token_address=token
            )
            # time.sleep(1)
        ...
        for order in carbon_orders:
            if not check_if_tkn_in_table_address(
                order.tkn0.address
            ) or not check_if_tkn_in_table_address(order.tkn1.address):
        ...
        for pair in pair_keys:
            ...
            organized_list.append(
                {
                    "token0": tkn0,
                    "amt0": pair_dict.get(pair).get(tkn0),
                    "type0": denom0,
                    "token1": tkn1,
                    "amt1": pair_dict.get(pair).get(tkn1),
                    "type1": denom1,
                    "total": pair_dict.get(pair).get("total"),
                }
            )
    ...
    def create_routes_carbon(self):
        ...
        self.unique_pools += self.get_uni_v2_sushi_pools(
            pairs=pairs_with_one_bancor_v3_token,
            non_bancor_v3_tokens=non_bancor_v3_tokens,
        )
