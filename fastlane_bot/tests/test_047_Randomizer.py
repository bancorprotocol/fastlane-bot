...
from fastlane_bot.utils import rand_item
...
# ------------------------------------------------------------
    ...
    for num_of_items in range(5):
        arb_opp = rand_item(list_of_items=arb_opps, num_of_items=num_of_items)        
        assert len(arb_opp) == 5, f"[NB047 Randomizer], expected 1 arb back from randomizer with length of 5, found length of {len(arb_opp)}"
        assert isinstance(np.float64(arb_opp[0]), np.floating), f"[NB047 Randomizer], expected first value back from randomizer to be of type np.float64, found type {type(arb_opp[0])}"
        assert type(arb_opp[2]) == tuple, f"[NB047 Randomizer], expected third value back from randomizer to be of type tuple, found type {type(arb_opp[2])}"
...
# ------------------------------------------------------------
    ...
    # +
    arb_opps = [
        (2.6927646232907136, [{'cid': '0xe37abfaee752c24a764955cbb2d10c3c9f88472263cbd2c00ca57facb0f128fe', 'tknin': 'WETH-6Cc2', 'amtin': 0.003982724863828224, 'tknout': 'BNT-FF1C', 'amtout': -19.27862435251882, 'error': None}, {'cid': '3743106036130323098097120681749450326076-0', 'tknin': 'BNT-FF1C', 'amtin': 16.585859729228105, 'tknout': 'WETH-6Cc2', 'amtout': -0.003982724874543209, 'error': None}]),
    ...
    arb_opp = rand_item(list_of_items=arb_opps, num_of_items=3)
    assert any(iseq(arb_opp[0], profit) for profit in [8.465616944048316, 6.717558869249757, 5.438084583685771]), f"[NB047 Randomizer], expected randomizer to return top 3 most profitable arb opps, but it did not!"
    # -
