...
class ArbitrageFinderBase:
    ...
    def create_sort_order(self, sort_sequence):
        ...
        return {
            key: index for index, key in enumerate(sort_sequence) if key != "carbon_v1"
        }
    ...
    def calculate_profit(
        ...
        if src_token not in [
            ...
            price_curves = self.get_prices_simple(
                CCm, self.ConfigObj.WRAPPED_GAS_TOKEN_ADDRESS, fl_token_with_weth
            )
            sorted_price_curves = self.custom_sort(price_curves, sort_sequence)
            self.ConfigObj.logger.debug(
                f"[modes.base.calculate_profit sort_sequence] {sort_sequence}"
            )
            self.ConfigObj.logger.debug(
                f"[modes.base.calculate_profit price_curves] {price_curves}"
            )
            self.ConfigObj.logger.debug(
                f"[modes.base.calculate_profit sorted_price_curves] {sorted_price_curves}"
            )
            if len(sorted_price_curves) > 0:
                ...
                self.ConfigObj.logger.debug(
                    f"[modes.base.calculate_profit] {src_token, best_profit_fl_token, fltkn_eth_conversion_rate, best_profit_eth}"
                )
            else:
