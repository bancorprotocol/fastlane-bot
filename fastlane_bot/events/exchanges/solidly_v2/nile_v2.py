...
from fastlane_bot.data.abi import NILE_V2_FACTORY_ABI
from .base import SolidlyV2


@dataclass
class NileV2(SolidlyV2):
    @property
    def factory_abi(self):
        return NILE_V2_FACTORY_ABI

    async def get_fee(self, address: str, contract: AsyncContract) -> Tuple[str, float]:
        fee = await self.factory_contract.caller.pairFee(address)
