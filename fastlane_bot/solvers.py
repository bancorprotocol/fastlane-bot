...
from fastlane_bot.utils import (
    swap_bancor_eth_to_weth,
    convert_to_correct_decimal,
    _quantize,
)
...
class CarbonV1RouteSolver(BaseRouteSolver):
    ...
    *****************************************************************************
    ...
    def simulate(
        self, trade_path: List[LiquidityPool] = None
        ...
        trade_amts = self.optimal_arb_carbon_triangular_constant_product(
            p1=p1, p2=p2, p3=p3
        )
    ...
    def get_trade_amts_carbon_triangular(
        ...
        tkns_in_carbon, tkns_out_carbon = self.get_output_trade_by_source(
            tkns_in=result_trade_1, order=p2
        )

        if result_trade_1 > p2.max_in:
            # traded in more than max, recalculating

            new_tkns_in = self.get_in_given_out_constant_product(
                tokens_out=tkns_in_carbon,
                token0_amt=p1.tkn0.amt,
        ...
        result_trade_3 = self.single_trade_result_constant_product_accurate(
            tokens_in=tkns_out_carbon,
            token0=p3.tkn0,
            token1=p3.tkn1,
            pool_fee=Decimal(p3.fee),
            token_in=p2.tkn1,
            token_out=p3.tkn1,
        )
        logger.info(
    ...
    def get_optimal_input_triangular_constant_product(
        ...
        trade_input = Decimal(
            (z * (-p1t0 * p3t0 * z + Decimal.sqrt(C * p1t0 * p3t0 * p1t1 * p3t1)))
            / (p1t1 * C + p1t1 * D * p3t0 + z**2 * p3t0)
        )
        return trade_input

    def get_output_trade_by_source(
        self, order: CarbonV1Order, tkns_in: Decimal
    ) -> Tuple[Decimal, Decimal]:
        """
        ...
        if tkns_out > y:
            tkns_in, tkns_out = CarbonV1RouteSolver.get_input_trade_by_target(
                order=order, tkns_out=y
            )
            tkns_out = y
    ...
    @staticmethod
    def get_input_trade_by_target(
        order: CarbonV1Order, tkns_out: Decimal
    ) -> Tuple[Decimal, Decimal]:
        """
    ...
    def single_trade_result_constant_product(
        self, tokens_in, token0_amt, token1_amt, pool_fee
    ) -> Decimal:
        """
        This function returns the output of a trade (-Dy) given Dx in a constant product pool
    ...
    def single_trade_result_constant_product_accurate(
        self,
        tokens_in: Decimal,
        token0: ERC20Token,
        token1: ERC20Token,
        ...
        token0_in = swap_bancor_eth_to_weth(token0.address) == swap_bancor_eth_to_weth(
            token_in.address
        )
        ...
        trade_result = (
            ...
            if token0_in
                ...
                tokens_in=tokens_in,
                token0_amt=token1.amt,
                token1_amt=token0.amt,
                pool_fee=pool_fee,
            )
        )
