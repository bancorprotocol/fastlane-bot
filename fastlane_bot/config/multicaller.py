...
class MultiCaller(ContextManager):
    ...
    def __init__(
        ...
        self.web3 = web3
        self.MULTICALL_CONTRACT_ADDRESS = self.web3.to_checksum_address(
            multicall_address
        )
    ...
    def multicall(self) -> List[Any]:
        ...
        for fn_list in function_keys:
            _encoded_data.append(
                self.web3.eth.contract(
                    abi=MULTICALL_ABI, address=self.MULTICALL_CONTRACT_ADDRESS
                )
                .functions.aggregate(_calls_for_aggregate[fn_list])
                .call(block_identifier=self.block_identifier)
            )

        if not isinstance(_encoded_data[0], list):
            raise TypeError(
                f"Expected encoded_data to be a list, got {type(_encoded_data[0])} instead."
            )
        ...
        if (
            "tokenPrice" in function_keys
            and "amountAvailableForTrading" in function_keys
        ):
            new_return = []
            ...
            assert (
                returned_items % 2 == 0
            ), f"[multicaller.py multicall] non-even number of returned calls for Bancor POL {returned_items}"
