...
def organize_pool_details_uni_v3(
    ...
    if "tkn0_address" in pool_data["args"]:
        ...
        print(
            f"failed to get tkn0_address for exchange: {exchange} from pool_data: {pool_data}"
        )
        assert False
    if len(tokens) > 2:
        return None
...
def terraform_blockchain(
    ...
    for row in multichain_df.iterrows():
        ...
        if fork in "uniswap_v2":
            ...
            factory_abi = (
                ...
                if exchange_name not in ["pancakeswap_v3"]
                else PANCAKESWAP_V3_FACTORY_ABI
            )
            factory_contract = web3.eth.contract(
                address=factory_address, abi=factory_abi
            )
    ...
    solidly_v2_mapdf.to_csv(
        (write_path + "/solidly_v2_event_mappings.csv"), index=False
    )
