...
class _DCBase():
    ...
    def asdict(self, *, exclude=None, include=None, dct=None):
        ...
        """
        if dct is None: dct = asdict(self)
...
class Path(_DCBase):
    ...
    def __post_init__(self):
       if not self.graph is None:
            assert isinstance(self.graph, ArbGraph), f"graph must be an ArbGraph, not {type(self.graph)}"
...
class ArbGraph(_DCBase):
    ...
    def from_r(cls, r, *, ignorezero=True, verbose=False):
        ...
        """
        return cls.from_ccdxdy(r.curves, r.dxvalues, r.dyvalues, ignorezero=ignorezero, verbose=False)
    ...
    class RunArbCycleResult(_DCBase):
        ...
        def asdict(self, *, include_cycle=True, exclude=None, include=None):
            dct =  {**asdict(self), "tokens": self.tokens(), "length": self.length, "r": self.r, 
