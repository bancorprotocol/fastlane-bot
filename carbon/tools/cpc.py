...
class ConstantProductCurve():
    ...
    def from_kx(cls, k, x, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=k, x=x, x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="kx", params=params)
    ...
    def from_ky(cls, k, y, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=k, x=k/y, x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="ky", params=params) 
    ...
    def from_xy(cls, x, y, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=x*y, x=x, x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="xy", params=params)
    ...
    def from_pk(cls, p, k, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=k, x=sqrt(k/p), x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="pk", params=params)
    ...
    def from_px(cls, p, x, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=x*x*p, x=x, x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="px", params=params)
    ...
    def from_py(cls, p, y, x_act=None, y_act=None, pair=None, cid=None, fee=None, descr=None, params=None):
        ...
        return cls(k=y*y/p, x=y/p, x_act=x_act, y_act=y_act, pair=pair, cid=cid, fee=fee, descr=descr, constr="py", params=params)
    
    @classmethod
    def from_pkpp(cls, p, k, p_min=None, p_max=None, pair=None, cid=None, fee=None, descr=None, *, constr=None, params=None):
        ...
        return cls(k=k, x=x0, x_act=xa, y_act=ya, pair=pair, cid=cid, fee=fee, descr=descr, constr="pkpp", params=params)
    ...
    def from_univ2(cls, x_tknb=None, y_tknq=None, k=None, pair=None, fee=None, cid=None, descr=None, params=None):
        ...
        *exactly one of k,x,y must be None; all other parameters must not be None;
        ...
        return cls(k=k, x=x, x_act=x, y_act=y, pair=pair, cid=cid, fee=fee, descr=descr, constr="uv2", params=params)
...
class CPCContainer():
    ...
    PE_CURVES = "curves"
    PE_DATA = "data"
    def price_estimate(self, *, tknq=None, tknb=None, pair=None, result=None, raiseonerror=True):
        """
        ...
        """
        assert (tknq is not None and tknb is not None) or pair is not None, f"must specify tknq, tknb or pair [{tknq}, {tknb}, {pair}]"
        ...
        if result==self.PE_PAIR:
            return f"{tknb}/{tknq}"
        crvs  = (c for c in self if not c.at_boundary and c.tknq==tknq and c.tknb==tknb)
        rcrvs = (c for c in self if not c.at_boundary and c.tknb==tknq and c.tknq==tknb)
    ...
    def price_estimates(self, *, tknqs=None, tknbs=None, triangulate=False, unwrapsingle=True, pairs=False, raiseonerror=True):
        """
        ...
        result = np.array([[
            self.price_estimate(tknb=b, tknq=q, raiseonerror=False, result=self.PE_PAIR if pairs else None) 
                            for b in tknbs] for q in tknqs])
        ...
        if raiseonerror:
            missing = { f"{b}/{q}" for ib,b in enumerate(tknbs) 
                       for iq,q in enumerate(tknqs) if result[iq][ib] is None }
            #print("[price_estimates] result", result)
            if not len(missing) == 0:
                raise ValueError(f"no price found for {len(missing)} pairs", result, missing, len(missing))
    ...
    def plot(self, *, pairs=None, directed=False, curves=None, params=None):
        ...
        for pair in pairs:
            ...
            for c in curves:
                ...
                label = None if not p.label else p.label.format(c=c, p=AD(dct=c.params), pinv=1/c.p)
