...
from fastlane_bot.tools.cpc import (
...
market_df = pd.read_csv("tests/_data/NBTEST_002_Curves.csv.gz")
...
def test_bycids():
    ...
    assert (
        raises(CC.bycids, include="foo", endswith="bar")
        == "include and endswith cannot be used together"
    )
...
def test_buysell():
    ...
    assert (
        c1.buysell(verbose=True, withprice=True) == "sell-WETH @ 2000.00 USDC per WETH"
    )
    ...
    # selling ETH at 2000-2001 USDC per ETH
    c1 = CPC.from_carbon(
        pair="WETH/USDC", tkny="WETH", yint=10, y=10, pa=2000, pb=2001, isdydx=False
    )
    assert c1.pair == "USDC/WETH"
    assert c1.primary == "WETH/USDC"
    assert c1.pairo.isprimary == False
    ...
    c2 = CPC.from_carbon(
        pair="WETH/USDC", tkny="USDC", yint=10, y=10, pa=1500, pb=1499, isdydx=True
    )
    assert c2.pair == "WETH/USDC"
    assert c2.primary == "WETH/USDC"
    assert c2.pairo.isprimary == True
    assert (
        c2.buysell(verbose=True, withprice=True) == "buy-WETH @ 1500.00 USDC per WETH"
    )
    ...
    # buying ETH at 1500-1499 USDC per ETH
    c2 = CPC.from_carbon(
        pair="WETH/USDC", tkny="USDC", yint=10, y=10, pa=1500, pb=1499, isdydx=False
    )
    assert c2.pair == "WETH/USDC"
    assert c2.primary == "WETH/USDC"
    assert c2.pairo.isprimary == True
    ...
    assert (
        c3.buysell(verbose=True, withprice=True)
        == "buy-sell-WETH @ 1900.00 USDC per WETH"
    )
    ...
    assert c3.buysell(verbose=False) == "bs"
    assert c3.buysell(verbose=False, withprice=True) == ("bs", 1900.0)
    assert c3.buysell() == "bs"
    ...
    assert (
        c4.buysell(verbose=True, withprice=True) == "sell-WETH @ 1899.00 USDC per WETH"
    )
    ...
    assert (
        c5.buysell(verbose=True, withprice=True) == "buy-WETH @ 1901.00 USDC per WETH"
    )
    ...
    c6 = CPC.from_univ2(
        pair="USDC/WETH", x_tknb=2000, y_tknq=1, cid="", fee=0, descr=""
    )
    assert c6.pair == "USDC/WETH"
    assert c6.primary == "WETH/USDC"
    assert c6.pairo.isprimary == False
    assert (
        c6.buysell(verbose=True, withprice=True)
        == "buy-sell-WETH @ 2000.00 USDC per WETH"
    )
    assert c6.buysell(verbose=False) == "bs"
    assert c6.buysell(verbose=False, withprice=True) == ("bs", 2000.0)
    assert c6.buysell() == "bs"

    # univ2 (tknq=2000 USDC, tknb=1 ETH)
    c7 = CPC.from_univ2(
        pair="WETH/USDC", x_tknb=1, y_tknq=2000, cid="", fee=0, descr=""
    )
    assert c7.pair == "WETH/USDC"
    assert c7.primary == "WETH/USDC"
    assert c7.pairo.isprimary == True
    assert (
        c7.buysell(verbose=True, withprice=True)
        == "buy-sell-WETH @ 2000.00 USDC per WETH"
    )
    assert c7.buysell(verbose=False) == "bs"
    assert c7.buysell(verbose=False, withprice=True) == ("bs", 2000.0)
...
def test_p():
    ...
    c = CPC.from_pk(
        pair="USDC/WETH", p=1, k=100, params=dict(exchange="univ3", a=dict(b=1, c=2))
    )
...
def test_byparams():
    ...
    pair = "USDC/WETH"
    c = [
        CPC.from_pk(pair=pair, p=1, k=100, params=dict(exchange="univ3", foo=1))
        for _ in range(5)
    ]
    c += [
        CPC.from_pk(pair=pair, p=1, k=100, params=dict(exchange="carbv1", foo=2))
        for _ in range(15)
    ]
    CC = CPCContainer(c)
    assert len(CC) == 20
    ...
    assert (
        type(CC.byparams(exchange="meh", _ascc=True, _asgenerator=True)).__name__
        == "generator"
    )
    assert (
        type(CC.byparams(exchange="meh", _ascc=False, _asgenerator=True)).__name__
        == "generator"
    )
    ...
    assert (
        raises(CC.byparams, foo=1, bar=2)
        == "currently only one param allowed {'foo': 1, 'bar': 2}"
    )
...
def test_itm():
    ...
    c1 = CPC.from_carbon(
        pair="WETH/USDC", tkny="USDC", yint=10, y=10, pb=2000, pa=2001, isdydx=False
    )
    c2 = CPC.from_carbon(
        pair="WETH/USDC", tkny="WETH", yint=10, y=10, pb=1500, pa=1499, isdydx=False
    )
    bs1 = c1.buysell(verbose=False, withprice=True)
    bs2 = c2.buysell(verbose=False, withprice=True)
    ...
    c2b = CPC.from_carbon(
        pair="WETH/USDC", tkny="WETH", yint=10, y=10, pb=2500, pa=2499, isdydx=False
    )
...
def test_estimate_prices():
    ...
    CC += [
        CPC.from_pk(
            pair="WETH/USDC", cid="uv2", fee=0, descr="", p=1950, k=5**2 * 2000
        )
    ]
    CC += [
        CPC.from_pk(
            pair="USDC/WETH", cid="uv2r", fee=0, descr="", p=1 / 1975, k=5**2 * 2000
        )
    ]
    CC += [
        CPC.from_carbon(
    ...
    assert (
        CC.price_estimate(tknq=T.WETH, tknb=T.USDC, result=CC.PE_PAIR)
        == f"{T.USDC}/{T.WETH}"
    )
    assert (
        CC.price_estimate(pair=f"{T.USDC}/{T.WETH}", result=CC.PE_PAIR)
        == f"{T.USDC}/{T.WETH}"
    )
    ...
    assert (
        CC.price_estimates(
            tknqs=[T.WETH], tknbs=[T.USDC], pairs=True, unwrapsingle=False
        )[0][0]
        == f"{T.USDC}/{T.WETH}"
    )
    assert (
        CC.price_estimates(
            tknqs=[T.WETH], tknbs=[T.USDC], pairs=True, unwrapsingle=True
        )[0]
        == f"{T.USDC}/{T.WETH}"
    )
    assert (
        CC.price_estimates(tknqs=[T.WETH], tknbs=[T.USDC], pairs=True)[0]
        == f"{T.USDC}/{T.WETH}"
    )
    ...
    assert O.price_estimates(tknq="USDC", tknbs=["WETH"]) == CC.price_estimates(
        tknqs=["USDC"], tknbs=["WETH"]
    )
...
def test_triangle_estimates():
    ...
    assert (
        CC.price_estimate(tknb=T.WETH, tknq=T.USDC, result=CC.PE_PAIR)
        == f"{T.WETH}/{T.USDC}"
    )
    ...
    r = CC.price_estimates(
        tknqs=[T.WETH], tknbs=[T.WBTC], unwrapsingle=True, pairs=True
    )
    assert r.shape == (1,)
    ...
    r = CC.price_estimates(
        tknqs=[T.WETH], tknbs=[T.WBTC], unwrapsingle=False, pairs=True
    )
    assert r.shape == (1, 1)
    ...
    assert raises(
        CC.price_estimates, tknqs=[T.WETH], tknbs=[T.WBTC], triangulate=False
    ).startswith("('no price found")
    r = CC.price_estimates(
        tknqs=[T.WETH], tknbs=[T.WBTC], raiseonerror=False, triangulate=False
    )
    assert r == CC.price_estimates(
        tknqs=[T.WETH], tknbs=[T.WBTC], raiseonerror=False, triangulate=False
    )
    assert r.shape == (1,)
    assert r[0] is None
...
def test_new_cpc_features_in_v2():
    ...
    pairs = [
        f"{a}/{b}"
        for a in ["ETH", "USDC", "DAI"]
        for b in ["DAI", "WBTC", "LINK", "ETH"]
        if a != b
    ]
    CC = CPCContainer()
    ...
    assert fp(tknbnotin="WBTC, ETH, DAI") == {
        "USDC/DAI",
        "USDC/ETH",
        "USDC/LINK",
        "USDC/WBTC",
...
def test_real_data_and_retrieval_of_curves():
    ...
    assert (
        CC.tokens_s()[:60]
        == "1INCH,1ONE,AAVE,ALCX,ALEPH,ALPHA,AMP,ANKR,ANT,APW,ARCONA,ARM"
    )
    ...
    assert CC.bypairs(CC.fp(onein="WETH, WBTC")) == CC.bypairs(
        CC.fp(onein="WETH, WBTC"), asgenerator=False
    )
...
def test_xyfromp_f_and_dxdyfromp_f():
    ...
    # +
    c = CPC.from_pkpp(
        p=100, k=100 * 10000, p_min=90, p_max=110, pair=f"{T.ETH}/{T.USDC}"
    )
    ...
    assert c.xyfromp_f(withunits=True) == (
        100.0,
        10000.0,
        100.0,
        T.WETH,
    ...
    # +
    assert c.dxdyfromp_f(withunits=True) == (
        0.0,
        0.0,
        100.0,
        T.WETH,
...
def test_simple_optimizer():
    ...
    CC = CPCContainer(
        CPC.from_pk(p=2000 + i * 10, k=10 * 20000, pair=f"ETH/USDC") for i in range(11)
    )
    c0 = CC.curves[0]
    c1 = CC.curves[-1]
    CC0 = CPCContainer([c0])
...
def test_optimizer_plus_inverted_curves():
    ...
    CCr = CPCContainer(
        CPC.from_pk(p=2000 + i * 100, k=10 * (20000 + 10000 * i), pair=f"ETH/USDC")
        for i in range(11)
    )
    CCi = CPCContainer(
        CPC.from_pk(
            p=1 / (2050 + i * 100), k=10 * (20000 + 10000 * i), pair=f"USDC/ETH"
        )
        for i in range(11)
    )
    CC = CCr.bycids()
...
def test_tradeinstructions():
    ...
    assert raises(
        TI, cid="1", tknin="USDC", amtin=2000, tknout="ETH", amtout=1, raiseonerror=True
    )
    assert raises(
        TI,
        cid="1",
    ...
    assert raises(
        TI, cid="1", tknin="USDC", amtin=2000, tknout="ETH", amtout=0, raiseonerror=True
    )
    assert raises(
        TI, cid="1", tknin="USDC", amtin=0, tknout="ETH", amtout=-1, raiseonerror=True
    )
    assert not raises(
        TI.new,
        curve_or_cid="1",
    ...
    assert not TI(
        cid="1", tknin="USDC", amtin=2000, tknout="ETH", amtout=-1, raiseonerror=False
    ).error
    assert TI(
        cid="1", tknin="USDC", amtin=2000, tknout="ETH", amtout=1, raiseonerror=False
    ).error
    ...
    assert TI(
        cid="1", tknin="USDC", amtin=2000, tknout="ETH", amtout=0, raiseonerror=False
    ).error
    assert TI(
        cid="1", tknin="USDC", amtin=0, tknout="ETH", amtout=-1, raiseonerror=False
    ).error
    assert not TI.new(
        curve_or_cid="1",
        tkn1="USDC",
    ...
    assert TI.new(
        curve_or_cid="1", tkn1="USDC", amt1=2000, tkn2="ETH", amt2=1, raiseonerror=False
    ).error
    assert TI.new(
        curve_or_cid="1",
        tkn1="USDC",
    ...
    assert TI.new(
        curve_or_cid="1", tkn1="USDC", amt1=0, tkn2="ETH", amt2=1, raiseonerror=False
    ).error
    assert TI.new(
        curve_or_cid="1",
        tkn1="USDC",
...
def test_margp_optimizer():
    ...
    f = O.optimize(
        "WETH", result=O.MO_DTKNFROMPF, params=dict(verbose=True, debug=False)
    )
    ...
    assert iseq(r.p_optimal_t[0], 0.0005421803152482512) or iseq(
        r.p_optimal_t[0], 0.00045575394031021585
    )
    assert iseq(r.p_optimal_t[1], 0.0005421803152482512) or iseq(
        r.p_optimal_t[1], 0.00045575394031021585
    )
    assert tuple(r.p_optimal.values())[:-1] == r.p_optimal_t
    assert set(r.tokens_t) == set(("USDC", "USDT"))
    assert r.errormsg is None
...
def notest_simple_optimizer_demo():
    ...
    CC = CPCContainer(
        CPC.from_pk(
            p=2000 + i * 100, k=10 * (20000 + i * 10000), pair=f"{T.ETH}/{T.USDC}"
        )
        for i in range(11)
    )
...
def notest_optimizer_plus_inverted_curves():
    ...
    CCr = CPCContainer(
        CPC.from_pk(
            p=2000 + i * 100, k=10 * (20000 + 10000 * i), pair=f"{T.ETH}/{T.USDC}"
        )
        for i in range(11)
    )
    CCi = CPCContainer(
        CPC.from_pk(
            p=1 / (2050 + i * 100), k=10 * (20000 + 10000 * i), pair=f"{T.USDC}/{T.ETH}"
        )
        for i in range(11)
    )
    CC = CCr.bycids()
...
def notest_operating_on_leverage_ranges():
    ...
    k = 100000**2 / (pb * pq)
    CCm += CPC.from_pk(
        p=pp, k=k, pair=pair, cid=f"mkt-{pair}", params=dict(xc="market")
    )
...
def notest_arbitrage_testing():
    ...
    c1 = CPC.from_pkpp(
        p=95, k=100 * 10000, p_min=90, p_max=110, pair=f"{T.ETH}/{T.USDC}"
    )
    c2 = CPC.from_pkpp(
        p=105, k=90 * 10000, p_min=90, p_max=110, pair=f"{T.ETH}/{T.USDC}"
    )
    CC = CPCContainer([c1, c2])
    CC.plot()
