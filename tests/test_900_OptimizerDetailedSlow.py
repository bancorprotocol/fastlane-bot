...
CCm = CPCContainer.from_df(pd.read_csv("tests/_data/NBTest_006.csv.gz"))
...
def notest_market_structure_analysis():
    ...
    CCx2 = CCx.bypairs(CCx.filter_pairs(onein=f"{T.HEX}, {T.UNI}, {T.ICHI}, {T.FRAX}"))
    ...
    O = MargPOptimizer(
        CCm.bypairs(
            CCm.filter_pairs(
                bothin=f"{T.ETH},{T.USDC},{T.USDT},{T.BNT},{T.DAI},{T.WBTC}"
            )
        )
    )
...
def test_abc_tests():
    ...
    assert raises(OptimizerBase.OptimizerResult).startswith(
        "Can't instantiate abstract class"
    )

    assert raises(CPCArbOptimizer).startswith("Can't instantiate abstract class")
    assert raises(CPCArbOptimizer.OptimizerResult).startswith(
        "Can't instantiate abstract class"
    )
...
def test_general_and_specific_tests():
    ...
    assert CA.pairsc() == {
        c.pairo.primary for c in CCm if c.P("exchange") == "carbon_v1"
    }
    ...
    assert set(len(x) for x in r1a.values()) == {
        2
    }, "all records must be of of length 2"
    assert set(type(x[0]) for x in r1a.values()) == {
        float
    }, "all records must have first type float"
    assert set(type(x[1]) for x in r1a.values()) == {
        str
    }, "all records must have second type str"
    ...
    assert tuple(df.loc["1701411834604692317316873037158841057339-0"]) == (
        1799.9999997028303,
        "WETH-6Cc2/USDC-eB48",
    )
    ...
    assert set(df["p_marg"]) == set(
        x[-1] for x in CAt.price_ranges(result=CCt.PR_TUPLE)
    )
    ...
    for i in range(10):
        assert len(CA.count_by_pairs(minn=i)) >= len(
            CA.count_by_pairs(minn=i)
        ), f"failed {i}"
    ...
    assert set(r.tokens_t) == set(
        ["USDT-1ec7", "WETH-6Cc2", "WBTC-C599", "DAI-1d0F", "BNT-FF1C"]
    )
    ...
    assert p_opt_d == {
        "WETH-6Cc2": 1842.67228,
        "WBTC-C599": 27604.143472,
        "BNT-FF1C": 0.429078,
        "USDT-1ec7": 1.00058,
        "DAI-1d0F": 1.000179,
    }
    ...
    print(
        f"total gains: {sum(dfpg['gain_ttkn']):,.2f} {r.targettkn} [result={-r.result:,.2f}]"
    )
    ...
    assert set(r.token_table.keys()) == set(
        ["USDT-1ec7", "WETH-6Cc2", "LINK-86CA", "DAI-1d0F", "HEX-eb39"]
    )
    ...
    assert raises(r.trade_instructions, ti_format=O.TIF_DFAGGR).startswith(
        "TIF_DFAGGR not implemented for"
    )
    assert raises(r.trade_instructions, ti_format=O.TIF_DFPG).startswith(
        "TIF_DFPG not implemented for"
    )
    ...
    assert len(df) == 12
    assert set(x[0] for x in tuple(df.index)) == {
        "carbon_v1",
        "sushiswap_v2",
        "uniswap_v2",
        "uniswap_v3",
