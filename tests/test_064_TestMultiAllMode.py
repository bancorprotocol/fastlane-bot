...
def test_test_min_profit():
    ...
    ), f"[NBTest64 TestMultiPairwiseAll Mode], default_min_profit_gas_token must be <= 0.02 for this Notebook to run, currently set to {cfg.DEFAULT_MIN_PROFIT_GAS_TOKEN}"
...
def test_test_get_arb_finder():
    ...
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Expected arb_finder class name name = FindArbitrageMultiPairwiseAll, found {arb_finder.__name__}"
...
def test_test_combos_and_tokens():
    ...
    assert (
        type(all_tokens) == set
    ), f"[NBTest64 TestMultiPairwiseAll Mode] all_tokens is wrong data type. Expected set, found: {type(all_tokens)}"
    assert (
        type(combos) == list
    ), f"[NBTest64 TestMultiPairwiseAll Mode] combos is wrong data type. Expected list, found: {type(combos)}"
    assert (
        len(all_tokens) > 100
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Using wrong dataset, expected at least 100 tokens, found {len(all_tokens)}"
    assert (
        len(combos) > 1000
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Using wrong dataset, expected at least 100 combos, found {len(combos)}"
    # -
...
def test_test_expected_output():
    ...
    assert (
        len(r) >= 25
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Expected at least 25 arbs, found {len(r)}"
    assert (
        multi_carbon_count > 0
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Not finding arbs with multiple Carbon curves."
    assert (
        carbon_wrong_direction_count == 0
    ), f"[NBTest64 TestMultiPairwiseAll Mode] Expected all Carbon curves to have the same tkn in and tkn out. Mixing is currently not supported."
    # -
