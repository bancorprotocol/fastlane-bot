...
def test_test_min_profit():
    ...
    ), f"[TestMultiTriangleMode], default_min_profit_gas_token must be <= 0.0001 for this Notebook to run, currently set to {cfg.DEFAULT_MIN_PROFIT_GAS_TOKEN}"

    # ### Test_arb_mode_class

    arb_finder = bot._get_arb_finder("multi_triangle")
    assert (
        arb_finder.__name__ == "ArbitrageFinderTriangleMulti"
    ), f"[TestMultiTriangleMode] Expected arb_finder class name name = FindArbitrageMultiPairwise, found {arb_finder.__name__}"
...
def test_test_combos():
    ...
    combos = finder2.get_combos(
        flashloan_tokens=flashloan_tokens, CCm=CCm, arb_mode="multi_triangle"
    )
    assert (
        len(combos) >= 1225
    ), f"[TestMultiTriangleMode] Using wrong dataset, expected at least 1225 combos, found {len(combos)}"
    ...
    for arb in r:
        ...
        if len(best_trade_instructions_dic) > 3:
            ...
            for curve in best_trade_instructions_dic:
                if "-0" in curve["cid"] or "-1" in curve["cid"]:
                    if curve["tknin"] in [tkn_in, tkn_out] and curve["tknout"] in [
                        tkn_in,
                        tkn_out,
                    ]:
                        assert (
                            curve["tknin"] in tkn_in
                        ), f"[TestMultiTriangleMode] Finding Carbon curves in opposite directions - not supported in this mode."
                        assert (
                            curve["tknout"] in tkn_out
                        ), f"[TestMultiTriangleMode] Finding Carbon curves in opposite directions - not supported in this mode."

    assert (
        multi_carbon_count > 0
    ), f"[TestMultiTriangleMode] Not finding arbs with multiple Carbon curves."
    assert (
        len(r) >= 58
    ), f"[TestMultiTriangleMode] Expected at least 58 arbs, found {len(r)}"
    # -
...
def test_test_triangle_single():
    ...
    assert (
        arb_finder.__name__ == "ArbitrageFinderTriangleSingle"
    ), f"[TestMultiTriangleMode] Expected arb_finder class name name = ArbitrageFinderTriangleSingle, found {arb_finder.__name__}"
...
def test_test_find_arbitrage_single():
    ...
    assert (
        multi_carbon_count == 0
    ), f"[TestMultiTriangleMode] Expected 0 arbs with multiple Carbon curves for Triangle Single mode, found {multi_carbon_count}."
    assert (
        len(r) >= 58
    ), f"[TestMultiTriangleMode] Expected at least 58 arbs, found {len(r)}"
    # -
