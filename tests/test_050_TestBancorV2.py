...
def test_test_min_profit():
    ...
    ), f"[TestBancorV2Mode], default_min_profit_gas_token must be <= 0.02 for this Notebook to run, currently set to {cfg.DEFAULT_MIN_PROFIT_GAS_TOKEN}"
...
def test_test_combos_and_tokens():
    ...
    assert (
        type(all_tokens) == set
    ), f"[NBTest_50_TestBancorV2] all_tokens is wrong data type. Expected set, found: {type(all_tokens)}"
    assert (
        type(combos) == list
    ), f"[NBTest_50_TestBancorV2] combos is wrong data type. Expected list, found: {type(combos)}"
    assert (
        len(all_tokens) > 100
    ), f"[NBTest_50_TestBancorV2] Using wrong dataset, expected at least 100 tokens, found {len(all_tokens)}"
    assert (
        len(combos) > 1000
    ), f"[NBTest_50_TestBancorV2] Using wrong dataset, expected at least 100 combos, found {len(combos)}"
...
def test_test_expected_output_bancorv2():
    ...
    assert (
        len(r) >= 27
    ), f"[NBTest_50_TestBancorV2] Expected at least 27 arb opps, found {len(r)}"
    assert (
        len(arb_with_bancor_v2) >= 3
    ), f"[NBTest_50_TestBancorV2] Expected at least 3 arb opps with Bancor V2 pools, found {len(arb_with_bancor_v2)}"
    ...
    b2pools = [
        pool["anchor"] for pool in mgr.pool_data if pool["exchange_name"] in "bancor_v2"
    ]
    bancor_v2_converter_addresses = [
        pool["anchor"] for pool in state if pool["exchange_name"] in "bancor_v2"
    ]
    assert (
        arb_finder.__name__ == "FindArbitrageMultiPairwiseAll"
    ), f"[NBTest_50_TestBancorV2] Expected arb_finder class name name = FindArbitrageMultiPairwise, found {arb_finder.__name__}"
    assert (
        len(r) > 30
    ), f"[NBTest_50_TestBancorV2] Expected at least 30 arb opps, found {len(r)}"
    assert (
        len(arb_with_bancor_v2) >= 3
    ), f"[NBTest_50_TestBancorV2] Expected at least 3 arb opps with Bancor V2 pools, found {len(arb_with_bancor_v2)}"
    assert (
        encoded_trade_instructions[0].amtin_wei == flashloan_amount
    ), f"[NBTest_50_TestBancorV2] First trade in should match flashloan amount, {encoded_trade_instructions[0].amtin_wei} does not = {flashloan_amount}"
    assert (
        route_struct[0]["customAddress"] in bancor_v2_converter_addresses
        or route_struct[1]["customAddress"] in bancor_v2_converter_addresses
    ), f"[NBTest_50_TestBancorV2] customAddress for Bancor V2.1 trade must be converter token address, expected: anchor for Bancor V2 pool for one address, found: {route_struct[0]['customAddress']} and {route_struct[1]['customAddress']}"
    # -
