...
mocked_contract.functions.getSwapFeePercentage.call = AsyncMock(
    return_value="10000000000000000" or 0.01
)
...
def test_test_solidly_v2_exchange():
    ...
    solidly_v2_exchange = SolidlyV2(
        exchange_name="solidly_v2", fee="0.003", router_address="jeffs_router"
    )
    ...
    async def test_uniswap_v2_exchange():
        ...
        assert (
            await solidly_v2_exchange.get_tkn0("", mocked_contract, None)
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_solidly_v2_exchange_fork():
    ...
    # +
    velocimeter_v2_exchange = SolidlyV2(
        exchange_name="velocimeter_v2", fee="0.0025", router_address="jjs_router"
    )

    @pytest.mark.asyncio
    async def test_uniswap_v2_exchange():
        assert (
            velocimeter_v2_exchange.exchange_name in "velocimeter_v2"
        ), f"Wrong exchange name. Expected velocimeter_v2, got {velocimeter_v2_exchange.exchange_name}"
        assert (
            velocimeter_v2_exchange.base_exchange_name in "solidly_v2"
        ), f"Wrong base exchange name. Expected solidly_v2, got {velocimeter_v2_exchange.base_exchange_name}"
        ...
        assert (
            await velocimeter_v2_exchange.get_tkn0("", mocked_contract, None)
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_uniswap_v2_exchange():
    ...
    async def test_uniswap_v2_exchange():
        ...
        assert await uniswap_v2_exchange.get_fee("", mocked_contract) == (
            "0.003",
            0.003,
        ), f"{await uniswap_v2_exchange.get_fee('', mocked_contract)}"
        assert (
            await uniswap_v2_exchange.get_tkn0("", mocked_contract, None)
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_uniswap_v2_exchange_fork():
    ...
    pancake_v2_exchange = UniswapV2(
        exchange_name="pancakeswap_v2", fee="0.0025", router_address="freds_router"
    )

    @pytest.mark.asyncio
    async def test_uniswap_v2_exchange_fork():
        assert (
            pancake_v2_exchange.exchange_name in "pancakeswap_v2"
        ), f"Wrong exchange name. Expected pancakeswap_v2, got {pancake_v2_exchange.exchange_name}"
        assert (
            pancake_v2_exchange.base_exchange_name in "uniswap_v2"
        ), f"Wrong base exchange name. Expected uniswap_v2, got {pancake_v2_exchange.base_exchange_name}"
        assert pancake_v2_exchange.get_abi() == PANCAKESWAP_V2_POOL_ABI
        assert await pancake_v2_exchange.get_fee("", mocked_contract) == (
            "0.0025",
            0.0025,
        ), f"{await uniswap_v2_exchange.get_fee('', mocked_contract)}"
        assert (
            await pancake_v2_exchange.get_tkn0("", mocked_contract, None)
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_uniswap_v3_exchange():
    ...
    async def test_uniswap_v3_exchange():
        ...
        assert await uniswap_v3_exchange.get_fee("", mocked_contract) == (
            await mocked_contract.functions.fee().call(),
            (float(await mocked_contract.functions.fee().call()) / 1000000.0),
        )
        assert (
            await uniswap_v3_exchange.get_tkn0("", mocked_contract, {})
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_uniswap_v3_exchange_fork():
    ...
    # +
    pancake_v3_exchange = UniswapV3(
        exchange_name="pancakeswap_v3", router_address="bobs_router"
    )

    @pytest.mark.asyncio
    async def test_uniswap_v3_exchange():
        assert (
            pancake_v3_exchange.exchange_name in "pancakeswap_v3"
        ), f"Wrong exchange name. Expected pancakeswap_v3, got {pancake_v3_exchange.exchange_name}"
        assert (
            pancake_v3_exchange.base_exchange_name in "uniswap_v3"
        ), f"Wrong base exchange name. Expected uniswap_v3, got {pancake_v3_exchange.base_exchange_name}"
        assert pancake_v3_exchange.get_abi() == PANCAKESWAP_V3_POOL_ABI
        assert await pancake_v3_exchange.get_fee("", mocked_contract) == (
            await mocked_contract.functions.fee().call(),
            (float(await mocked_contract.functions.fee().call()) / 1000000.0),
        )
        assert (
            await pancake_v3_exchange.get_tkn0("", mocked_contract, {})
            == await mocked_contract.functions.token0().call()
        )
...
def test_test_bancor_v2_exchange():
    ...
    async def test_bancor_v2_exchange():
        ...
        assert (
            await bancor_v2_exchange.get_tkn0(
                "", mocked_contract, setup_data["bancor_v2_event"]
            )
...
def test_test_carbon_v1_exchange_update():
    ...
    async def test_carbon_v1_exchange_update():
        ...
        assert (
            await carbon_v1_exchange.get_tkn0(
...
def test_test_carbon_v1_exchange_delete():
    ...
    async def test_bancor_pol_exchange():
        ...
        assert (
            await bancor_pol_exchange.get_tkn0(
                "", mocked_contract, setup_data["bancor_pol_trading_enabled_event"]
            )
