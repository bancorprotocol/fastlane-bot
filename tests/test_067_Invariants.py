# ------------------------------------------------------------
# Auto generated test file `test_067_Invariants.py`
# ------------------------------------------------------------
# source file   = NBTest_067_Invariants.py
# test id       = 067
# test comment  = Invariants
# ------------------------------------------------------------
...
try:
    ...
    from fastlane_bot.tools.invariants.solidly import (
        SolidlyInvariant,
        SolidlySwapFunction,
    )
    from tests.testing import *
...
# ------------------------------------------------------------
# Test      067
# File      test_067_Invariants.py
# Segment   General invariants
# ------------------------------------------------------------
def test_general_invariants():
    # ------------------------------------------------------------
    ...
    inv.y_func(20, k=100), inv.y_func_from_k_func(20, k=100), inv.y_func_from_k_func(
        20, k=100, method=inv.GS_BISECT
    )
    ...
    r = (
        ...
        timer(
            inv.y_func_from_k_func,
            x=20,
            k=100,
            x_lo=0.1,
            x_hi=10,
            method=inv.GS_BISECT,
            N=10_000,
        ),
    )
    r, (1, r[1] / r[0], r[2] / r[0])
    ...
    r = timer2(inv.y_func, 20, 100, N=1000), timer2(
        inv.y_func_from_k_func, 20, 100, N=1000
    )
    r, r[1] / r[0]
