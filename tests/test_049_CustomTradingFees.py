...
def test_test_update_from_event_carbon_v1_trading_fee_updated():
    ...
    mocked_contract.functions.tradingFeePPM.return_value.call = AsyncMock(
        return_value=prevFeePPM
    )
    new_mocked_contract.functions.tradingFeePPM.return_value.call = AsyncMock(
        return_value=newFeePPM
    )
    ...
    async def test_update_from_event_carbon_v1_trading_fee_updated():
        ...
        prev_default_pools = [
            idx
            for idx, pool in enumerate(manager.pool_data)
            if pool["fee"] == prevFeePPM
        ]
...
def test_test_update_from_event_carbon_v1_pair_trading_fee_updated():
    ...
    others = [
        i
        for i, pool in enumerate(manager.pool_data)
        if i not in idxs and pool["exchange_name"] == "carbon_v1"
    ]
    for i in others:
