...
def test_test_min_profit():
    ...
    ), f"[test_bancor_v3_two_hop], default_min_profit_gas_token must be <= 0.0001 for this Notebook to run, currently set to {cfg.DEFAULT_MIN_PROFIT_GAS_TOKEN}"
...
def test_test_arb_mode_class():
    ...
    assert (
        arb_finder.__name__ == "ArbitrageFinderTriangleBancor3TwoHop"
    ), f"[test_bancor_v3_two_hop] Wrong Arb Finder class, expected ArbitrageFinderTriangleBancor3TwoHop, got {arb_finder.__name__}"
...
def test_test_trade_merge():
    ...
    assert (
        calculated_trade_instructions[0].tknin
        != "0x1F573D6Fb3F13d689FF844B4cE37794d79a7FF1C"
    )
    assert (
        calculated_trade_instructions[0].tknout
        != "0x1F573D6Fb3F13d689FF844B4cE37794d79a7FF1C"
    )
...
def test_test_get_optimal_arb_trade_amts():
    ...
    assert (
        first_check_pools[0].cid == pool_cids[0]
    ), f"[test_bancor_v3_two_hop] Validation, wrong first pool, expected CID: 0x7be3da0f8d0f70d8f7a84a08dd267beea4318ed1c9fb3d602b0f3a3c7bd1cf4a, got CID: {first_check_pools[0].cid}"
    assert (
        first_check_pools[1].cid == pool_cids[1]
    ), f"[test_bancor_v3_two_hop] Validation, wrong second pool, expected CID: 0x748ab2bef0d97e5a044268626e6c9c104bab818605d44f650fdeaa03a3c742d2, got CID: {first_check_pools[1].cid}"
    assert (
        first_check_pools[2].cid == pool_cids[2]
    ), f"[test_bancor_v3_two_hop] Validation, wrong third pool, expected CID: 0xb1d8cd62f75016872495dae3e19d96e364767e7d674488392029d15cdbcd7b34, got CID: {first_check_pools[2].cid}"
    assert (
        len(first_check_pools) == 3
    ), f"[test_bancor_v3_two_hop] Validation expected 3 pools, got {len(first_check_pools)}"
    for pool in first_check_pools:
        assert (
            type(pool) == ConstantProductCurve
        ), f"[test_bancor_v3_two_hop] Validation pool type mismatch, got {type(pool)} expected ConstantProductCurve"
        assert (
            pool.cid in pool_cids
        ), f"[test_bancor_v3_two_hop] Validation missing pool.cid {pool.cid} in {pool_cids}"

    optimal_arb = finder.get_optimal_arb_trade_amts(pool_cids, "DAI-1d0F")
    assert (
        type(optimal_arb) == float
    ), f"[test_bancor_v3_two_hop] Optimal arb calculation type is {type(optimal_arb)} not float"
    # -
...
def test_test_max_arb_trade_in_constant_product():
    ...
    tkn0 = flt
    tkn1 = (
        finder.get_tkn(pool=first_check_pools[0], tkn_num=1)
        if finder.get_tkn(pool=first_check_pools[0], tkn_num=1) != flt
        else finder.get_tkn(pool=first_check_pools[0], tkn_num=0)
    )
    tkn2 = (
        finder.get_tkn(pool=first_check_pools[1], tkn_num=0)
        if finder.get_tkn(pool=first_check_pools[1], tkn_num=0) == tkn1
        else finder.get_tkn(pool=first_check_pools[1], tkn_num=1)
    )
    tkn3 = (
        finder.get_tkn(pool=first_check_pools[1], tkn_num=0)
        if finder.get_tkn(pool=first_check_pools[1], tkn_num=0) != tkn1
    ...
    tkn5 = (
        finder.get_tkn(pool=first_check_pools[2], tkn_num=1)
        if finder.get_tkn(pool=first_check_pools[2], tkn_num=1) == flt
        else finder.get_tkn(pool=first_check_pools[2], tkn_num=0)
    )
    p0t0 = (
        first_check_pools[0].x
        if finder.get_tkn(pool=first_check_pools[0], tkn_num=0) == flt
        else first_check_pools[0].y
    )
    p0t1 = (
        first_check_pools[0].y
        if finder.get_tkn(pool=first_check_pools[0], tkn_num=0) == flt
        else first_check_pools[0].x
    )
    p1t0 = (
        first_check_pools[1].x
        if tkn1 == finder.get_tkn(pool=first_check_pools[1], tkn_num=0)
        else first_check_pools[1].y
    )
    p1t1 = (
        first_check_pools[1].y
        if tkn1 == finder.get_tkn(pool=first_check_pools[1], tkn_num=0)
        else first_check_pools[1].x
    )
    p2t0 = (
        first_check_pools[2].x
        if finder.get_tkn(pool=first_check_pools[2], tkn_num=0) != flt
        else first_check_pools[2].y
    )
    p2t1 = (
        first_check_pools[2].y
        if finder.get_tkn(pool=first_check_pools[2], tkn_num=0) != flt
        else first_check_pools[2].x
    )
    ...
    optimal_arb = finder.get_optimal_arb_trade_amts(
        pool_cids, "0x6B175474E89094C44Da98b954EedeAC495271d0F"
    )
    optimal_arb_low_level_check = finder.max_arb_trade_in_constant_product(
        p0t0=p0t0,
        p0t1=p0t1,
        p1t0=p1t0,
        p1t1=p1t1,
    ...
    assert iseq(
        optimal_arb, optimal_arb_low_level_check
    ), f"[test_bancor_v3_two_hop] Arb calculation result mismatch, pools likely ordered incorrectly, previous calc: {optimal_arb}, this calc: {optimal_arb_low_level_check}"
...
def test_test_get_fee_safe():
    ...
    for pool in first_check_pools:
        ...
        assert (
            type(ext_fee) == float
        ), f"[test_bancor_v3_two_hop] Testing external pool, fee type is {type(ext_fee)} not float"
    # -
...
def test_test_combos():
    ...
    assert (
        len(combos) >= 1122
    ), "[test_bancor_v3_two_hop] Different data used for tests, expected 1122 combos"
...
def test_test_get_miniverse_combos():
    ...
    assert (
        len(all_miniverses) >= 6
    ), f"[test_bancor_v3_two_hop] Different data used for tests, expected 6 miniverses, found {len(all_miniverses)}"
