...
uniswap_v3_event_mappings = pd.read_csv(
    "fastlane_bot/data/uniswap_v3_event_mappings.csv", low_memory=False
)
...
def test_test_precision_using_all_tokens_in_carbon():
    ...
    def calculate_trade_outputs(
        tx_route_handler: TxRouteHandler, trade_instructions: List[TradeInstruction]
        ...
        for idx, trade in enumerate(trade_instructions):
            ...
            if trade.raw_txs != "[]":
                ...
                tx_route_handler.ConfigObj.logger.info(
                    f"[calculate_trade_outputs Carbon] starting Carbon trade calculations, {len(data)} trades, remaining_tkn_in = {remaining_tkn_in}"
                )
                for tx in data:
                    try:
                ...
                for _idx, tx in enumerate(data):
                    ...
                    tx_route_handler.ConfigObj.logger.info(
                        f"[calculate_trade_outputs Carbon] calculated trade, {amount_in} {tknin_key} into trade, remaining={remaining_tkn_in}"
                    )
                    ...
                    if _idx == last_tx and remaining_tkn_in > 0:
                        tx_route_handler.ConfigObj.logger.info(
                            f"[calculate_trade_outputs Carbon] LAST trade going into Carbon but {remaining_tkn_in} remaining. Stuffing remainder into other orders:"
                        )
                        ...
                        for __idx, _tx in enumerate(raw_txs_lst):
                            ...
                            if __idx == last_tx:
                                assert (
                                    remaining_tkn_in == 0
                                ), f"Failed to use all tokens into Carbon trade after trying to put more tokens into every order, {remaining_tkn_in} remaining"
                ...
                assert (
                    remaining_tkn_in == 0
                ), f"Failed to use all tokens into Carbon trade, {remaining_tkn_in} remaining"
    ...
    for arb in r:
        ...
        # try:
        calculated_trade_instructions = calculate_trade_outputs(
            tx_route_handler=tx_route_handler, trade_instructions=agg_trade_instructions
        )
