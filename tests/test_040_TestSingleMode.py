...
), f"[TestSingleMode], default_min_profit_gas_token must be <= 0.02 for this Notebook to run, currently set to {cfg.DEFAULT_MIN_PROFIT_GAS_TOKEN}"
...
def test_test_arb_mode_class():
    ...
    assert (
        arb_finder.__name__ == "FindArbitrageSinglePairwise"
    ), f"[TestSingleMode] Expected arb_finder class name name = FindArbitrageSinglePairwise, found {arb_finder.__name__}"
...
def test_test_tokens_and_combos():
    ...
    assert (
        type(all_tokens) == set
    ), f"[TestSingleMode] all_tokens is wrong data type. Expected set, found: {type(all_tokens)}"
    assert (
        type(combos) == list
    ), f"[TestSingleMode] combos is wrong data type. Expected list, found: {type(combos)}"
    assert (
        len(all_tokens) > 100
    ), f"[TestSingleMode] Using wrong dataset, expected at least 100 tokens, found {len(all_tokens)}"
    assert (
        len(combos) > 1000
    ), f"[TestSingleMode] Using wrong dataset, expected at least 100 combos, found {len(combos)}"
    # -
    ...
    assert (
        multi_carbon_count == 0
    ), f"[TestSingleMode] Expected arbs without multiple Carbon curves, but found {len(multi_carbon_count)}"
    assert len(r) >= 20, f"[TestSingleMode] Expected at least 20 arbs, found {len(r)}"
    assert len(r) == len(
        run_full
    ), f"[TestSingleMode] Expected arbs from .find_arbitrage - {len(r)} - to match _run - {len(run_full)}"
    # -
