...
def test_test_combos_and_tokens():
    ...
    assert (
        type(all_tokens) == set
    ), f"[NBTest 039 TestMultiMode] all_tokens is wrong data type. Expected set, found: {type(all_tokens)}"
    assert (
        type(combos) == list
    ), f"[NBTest 039 TestMultiMode] combos is wrong data type. Expected list, found: {type(combos)}"
    assert (
        len(all_tokens) >= 236
    ), f"[NBTest 039 TestMultiMode] Using wrong dataset, expected at least 236 tokens, found {len(all_tokens)}"
    assert (
        len(combos) >= 1410
    ), f"[NBTest 039 TestMultiMode] Using wrong dataset, expected at least 1410 combos, found {len(combos)}"
    # -
...
def test_test_expected_output():
    ...
    assert (
        len(r) >= 27
    ), f"[NBTest 039 TestMultiMode] Expected at least 27 arbs, found {len(r)}"
    assert (
        multi_carbon_count > 0
    ), f"[NBTest 039 TestMultiMode] Not finding arbs with multiple Carbon curves."
    assert (
        carbon_wrong_direction_count == 0
    ), f"[NBTest 039 TestMultiMode] Expected all Carbon curves to have the same tkn in and tkn out. Mixing is currently not supported."
    # -
