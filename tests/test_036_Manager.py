...
(
    ...
    cfg,
    ["bancor_v3", "carbon_v1", "uniswap_v2", "uniswap_v3"],
    "ethereum",
    "static_pool_data",
)
manager = Manager(
...
def test_test_update_from_event_uniswap_v2():
    ...
    assert (
        ...
        != [
            pool["tkn0_balance"]
            for pool in manager.pool_data
            if pool["address"] == event["address"]
        ][0]
    )
...
def test_test_update_from_event_carbon_v1_update():
    ...
    pools_to_add_from_contracts = [
        event[2]["args"]["id"] for event in manager.pools_to_add_from_contracts
    ]
...
def test_test_update_from_event_carbon_v1_create():
    ...
    manager.pool_data = [
        pool for pool in manager.pool_data if pool["cid"] != event["args"]["id"]
    ]
