...
def test_arbgraphs_test_and_demo():
    ...
    assert (edge.pair(), edge.price(), edge.convention()) == (
        "ETH/USDC",
        2000.0,
        "USDC per ETH",
    )
    assert (edge1.pair(), edge1.price(), edge1.convention()) == (
        "USDC/ETH",
        0.0005,
        "ETH per USDC",
    )
...
def test_paths_and_cycles():
    ...
    assert c1.filter_subcycles(
        [ag.Cycle([8, 9]), ag.Cycle([1, 5, 6]), ag.Cycle([1, 6, 5])]
    ) == (ag.Cycle([1, 5, 6]),)
    ...
    assert list(c1.asdf(include=["data", "graph"], exclude=["graph"]).columns) == [
        "data"
    ]
...
def test_arbgraph_transport_test_and_demo_2():
    ...
    @ag.dataclass
    class MyState:
        myval_: ag.TrackedStateFloat = ag.field(
            default_factory=ag.TrackedStateFloat, init=False
        )
...
def test_transport_3_and_prices():
    ...
    for t1, p1 in prices.items():
        for t2, p2 in prices.items():
            if t1 < t2:
                AG.add_edge_ct(
                    tkn_in=t1, tkn_out=t2, price_outperin=p1 / p2, symmetric=True
                )
    ...
    assert BTC.tkn == "BTC"
    assert (
        AG.ptransport(ag.Path([USDC, WETH])).multiplier
        == prices[USDC.tkn] / prices[WETH.tkn]
    )
    assert (
        AG.ptransport(ag.Path([USDC, WETH, AAVE, LINK, BTC])).multiplier
        == AG.ptransport(ag.Path([USDC, BTC])).multiplier
    )
    for n1 in AG.nodes:
        for n2 in AG.nodes:
            if n1 != n2:
                assert (
                    AG.ptransport(ag.Path([n1, n2])).multiplier
                    == prices[n1.tkn] / prices[n2.tkn]
                )
    # AG.plot(rnone=True)
    ...
    assert (
        AG.ptransport(AG.shortest_path(USDC, WETH)).multiplier
        == prices[USDC.tkn] / prices[WETH.tkn]
    )
    assert (
        AG.ptransport(AG.shortest_path(BTC, WETH)).multiplier
        == prices[BTC.tkn] / prices[WETH.tkn]
    )
    assert AG.price(node_tknb=WETH, node_tknq=USDC) == AG.price(WETH, USDC)
    assert AG.price(WETH, USDC, with_units=True) == (
        2000.0,
        "USDC per WETH [WETH/USDC]",
    )
    assert AG.price(WETH, WETH, with_units=True) == (1, "WETH per WETH [WETH/WETH]")
    assert AG.price("WETH", "USDC", with_units=True) == AG.price(
        WETH, USDC, with_units=True
    )
    ...
    for n1 in AG.nodes:
        for n2 in AG.nodes:
            if n1 != n2:
                assert (
                    AG.ptransport(AG.shortest_path(n1, n2)).multiplier
                    == prices[n1.tkn] / prices[n2.tkn]
                )
                assert (
                    AG.price(node_tknb=n1, node_tknq=n2)
                    == prices[n1.tkn] / prices[n2.tkn]
                )
    # AG.plot(rnone=True)
...
def test_arbraph_connection_only_edges_test():
    ...
    e = e3 = ag.Edge.connection_edge(
        node_out=ETH, node_in=USDC, price=2000, inverse=True
    )
    assert e.convention() == "USDC per ETH"
    ...
    e = e4 = ag.Edge(
        node_in=ETH, node_out=USDC, amount_in=1, amount_out=2000, inverse=True
    )
    ...
    AG = ag.ArbGraph()
    AG.add_edge_connectiontype(
        tkn_in="ETH", tkn_out="USDC", price=2000, symmetric=False
    )
    AG.add_edge_connectiontype(
        tkn_in="ETH", tkn_out="BTC", price=1 / 5, symmetric=False
    )
    AG.add_edge_connectiontype(
        tkn_in="BTC", tkn_out="USDC", price=10000, symmetric=False
    )
    assert AG.edgetype == AG.EDGE_CONNECTION
    assert len(AG) == 3
    # _=AG.plot()
...
def test_interaction_with_cpc():
    ...
    c1 = CPC.from_univ2(
        x_tknb=1, y_tknq=2000, pair="ETH/USDC", fee=0, cid="1", descr="UniV2"
    )
    c2 = CPC.from_univ2(
        x_tknb=1, y_tknq=10000, pair="WBTC/USDC", fee=0, cid="2", descr="UniV2"
    )
    c3 = CPC.from_univ2(
        x_tknb=1, y_tknq=5, pair="WBTC/ETH", fee=0, cid="3", descr="UniV2"
    )
    assert c1.p == 2000
    assert c2.p == 10000
    assert c3.p == 5
...
def test_with_real_data_from_cpc():
    ...
    df = pd.read_csv("tests/_data/NBTEST_002_Curves.csv.gz")
    ...
    assert AG2.edge_statistics("WETH", "USDC", bothways=True)[1].asdict()[
        "amounts_in_remaining"
    ] == (755278.31, 1507698.66)
    ...
    assert AG2.filter_edges("WETH", "USDC", bothways=True) == AG2.filter_edges(
        "USDC", "WETH"
    )
    assert AG2.filter_edges(pair="WETH/USDC", bothways=False) == []
    assert AG2.filter_edges(pair="WETH/USDC") == AG2.filter_edges(
        "WETH", "USDC", bothways=True
    )
    assert AG2.filter_edges == AG2.fe
    assert AG2.fep("WETH/USDC") == AG2.filter_edges(pair="WETH/USDC")
    assert AG2.fep("WETH/USDC", bothways=False) == AG2.filter_edges(
        pair="WETH/USDC", bothways=False
    )
    assert tuple(AG2.edgedf(consolidated=True, resetindex=False).iloc[0]) == (
        1.41,
        0.02,
    )
