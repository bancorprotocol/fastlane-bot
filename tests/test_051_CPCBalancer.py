...
def test_constant_product_constructor():
    ...
    assert (
        raises(CPC.from_xyal, 100, 200, alpha=0.5, eta=1)
        == "at most one of alpha and eta must be given [0.5, 1]"
    )
...
def test_consistency_tests():
    ...
    for xy in np.logspace(1, 3, 100):
        for ci in cc:
            ...
            assert iseq(
                ci.yfromx_f(ci.xfromy_f(xy, ignorebounds=True), ignorebounds=True), xy
            )
            assert iseq(
                ci.xfromy_f(ci.yfromx_f(xy, ignorebounds=True), ignorebounds=True), xy
            )
    ...
    for alpha in np.linspace(0.01, 0.99, 100):
        ...
        dy = ci.yfromx_f(ci.x + 0.1, ignorebounds=True) - ci.yfromx_f(
            ci.x - 0.1, ignorebounds=True
        )
    ...
    for dxy in np.linspace(0.1, 99, 100):
        for ci in cc:
            assert iseq(
                ci.dyfromdx_f(dxy, ignorebounds=True),
                (ci.yfromx_f(ci.x + dxy, ignorebounds=True) - ci.y),
            )
            assert iseq(
                ci.dxfromdy_f(dxy, ignorebounds=True),
...
def notest_charts():
    ...
    for ci in cc:
        plt.plot(
            xvals,
            [ci.yfromx_f(x, ignorebounds=True) for x in xvals],
            label=f"eta={ci.eta:0.2f}",
        )
    plt.grid()
    plt.legend()
    plt.title(
        "Indifference curve token balance y vs token balance x at different weights"
    )
    ...
    for ci in cc:
        plt.plot(
            ...
            [
                -(
                    ci.yfromx_f(x + 0.1, ignorebounds=True)
                    - ci.yfromx_f(x - 0.1, ignorebounds=True)
                )
                / 0.2
                for x in xvals
    ...
    for ci in cc:
        plt.plot(
            pvals,
            [ci.xyfromp_f(p, ignorebounds=True)[1] for p in pvals],
            label=f"eta={ci.eta:0.2f}",
        )
    plt.grid()
    plt.legend()
