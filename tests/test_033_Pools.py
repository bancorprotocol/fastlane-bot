...
def test_test_solidly_v2_pool():
    ...
    solidly_v2_pool.update_from_event(
        setup_data["solidly_v2_event"],
        {
            "cid": "0x",
            "fee": "0.000",
            "fee_float": 0.0,
            "exchange_name": "velocimeter_v2",
            "reserve0": setup_data["solidly_v2_event"]["args"]["reserve0"],
            "reserve1": setup_data["solidly_v2_event"]["args"]["reserve1"],
            "tkn0_symbol": "tkn0",
            "tkn1_symbol": "tkn1",
        },
    )
    assert (
        solidly_v2_pool.state["tkn0_balance"]
        == setup_data["solidly_v2_event"]["args"]["reserve0"]
    )
...
def test_test_bancor_v2_pool():
    ...
    bancor_v2_pool.update_from_event(
        setup_data["bancor_v2_event"],
        {"cid": "0x", "fee": "0.000", "fee_float": 0.0, "exchange_name": "bancor_v2"},
    )
    assert (
        5698079648237338312679700 == setup_data["bancor_v2_event"]["args"]["_rateN"]
    ), f"expected {bancor_v2_pool.state['tkn0_balance']}, found {setup_data['bancor_v2_event']['args']['_rateN']}"
    assert (
        1404376232459809237924 == setup_data["bancor_v2_event"]["args"]["_rateD"]
    ), f"expected {bancor_v2_pool.state['tkn1_balance']}, found {setup_data['bancor_v2_event']['args']['_rateD']}"
...
def test_test_pancakeswap_v2_pool():
    ...
    pancakeswap_v2_pool.update_from_event(
        setup_data["pancakeswap_v2_event"],
        {
            "cid": "0x",
            "fee": "0.000",
            "fee_float": 0.0,
            "exchange_name": "pancakeswap_v2",
            "reserve0": setup_data["pancakeswap_v2_event"]["args"]["reserve0"],
            "reserve1": setup_data["pancakeswap_v2_event"]["args"]["reserve1"],
            "tkn0_symbol": "tkn0",
            "tkn1_symbol": "tkn1",
        },
    )
    assert (
        pancakeswap_v2_pool.state["tkn0_balance"]
        == setup_data["pancakeswap_v2_event"]["args"]["reserve0"]
    )
...
def test_test_pancakeswap_v3_pool():
    ...
    pancakeswap_v3_pool.update_from_event(
        setup_data["pancakeswap_v3_event"],
        {
            "cid": "0x",
            "fee": "0.000",
            "fee_float": 0.0,
            "exchange_name": "pancakeswap_v3",
            "liquidity": setup_data["pancakeswap_v3_event"]["args"]["liquidity"],
            "sqrtPriceX96": setup_data["pancakeswap_v3_event"]["args"]["sqrtPriceX96"],
            "tick": setup_data["pancakeswap_v3_event"]["args"]["tick"],
            "tkn0_symbol": "tkn0",
            "tkn1_symbol": "tkn1",
        },
    )
    assert (
        pancakeswap_v3_pool.state["liquidity"]
        == setup_data["pancakeswap_v3_event"]["args"]["liquidity"]
    )
    assert (
        pancakeswap_v3_pool.state["sqrt_price_q96"]
        == setup_data["pancakeswap_v3_event"]["args"]["sqrtPriceX96"]
    )
...
def test_test_bancor_pol_token_traded_event():
    ...
    bancor_pol_pool.state["tkn0_address"] = setup_data["bancor_pol_token_traded_event"][
        "args"
    ]["token"]
    bancor_pol_pool.state["tkn0_balance"] = (
        10 + setup_data["bancor_pol_token_traded_event"]["args"]["amount"]
    )
    bancor_pol_pool.update_from_event(
        setup_data["bancor_pol_token_traded_event"],
        {
            "cid": "0x",
            "fee": "0.000",
            "fee_float": 0.0,
            ...
            "ethReceived": setup_data["bancor_pol_token_traded_event"]["args"][
                "ethReceived"
            ],
            "tkn0_symbol": "tkn0",
            "tkn1_symbol": "tkn1",
...
def test_test_bancor_pol_trading_enabled_event():
    ...
    bancor_pol_pool.update_from_event(
        setup_data["bancor_pol_trading_enabled_event"],
        {
            "cid": "0x",
            "fee": "0.000",
            "fee_float": 0.0,
    ...
    assert (
        bancor_pol_pool.state["tkn0_address"]
        == setup_data["bancor_pol_trading_enabled_event"]["args"]["token"]
    )
