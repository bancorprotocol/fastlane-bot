...
Clone the repo from the Bancor github `main-v2` branch.

    git clone -b main-v2 https://github.com/bancorprotocol/fastlane-bot
    cd fastlane-bot
...
feat(main-v2): Introduce breaking changes with new infrastructure, Carbon exchange, and expanded trading capabilities

This commit introduces major breaking changes to the codebase, including new infrastructure requirements such as Postgres database and multiple new Python optimization libraries. The updated codebase will live on branch `main-v2` until further notice, allowing for sufficient testing and user notification before deprecating the v1 codebase currently on the `main` branch.

Notable changes in this commit include:

- Complete replacement of v1 functionality
- Support for arbitrage on the new Carbon exchange
- Support for trades of arbitrary route length
- Flash loan support in multiple token denominations (e.g., USDC, WBTC, WETH, etc.)

Please note that this commit will eventually be merged into the `main` branch once real-world testing and user transition have been deemed successful. For the time being, we will maintain the v1 codebase on the `main` branch, while the `main-v2` branch houses the new v2 codebase.

We encourage users to begin exploring the new v2 codebase and provide feedback on any issues or feature requests they may have. As always, thank you for your support and cooperation as we continue to improve our platform.
