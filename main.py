...
from fastlane_bot.config.constants import BLOCKCHAIN_DEFAULTS, ARGUMENTS
...
def setup_arguments(parser, arguments):
    for name, options in arguments:
        parser.add_argument(name, **options)

def apply_blockchain_defaults(args):
    blockchain = args.blockchain
    if blockchain not in BLOCKCHAIN_DEFAULTS.keys():
        raise ValueError("Blockchain defaults not established - please update in to constants.py")
    else:
        defaults = BLOCKCHAIN_DEFAULTS.get(blockchain, {})
    
    # Iterate over the defaults and set them if they weren't specified by the user
...
def main() -> None:
    ...
    """
    parser = argparse.ArgumentParser(description="Blockchain-specific configuration.")
    # Establish appropriate arguments
    setup_arguments(parser, ARGUMENTS)
    args = parser.parse_args()
    # Apply all other default arguments for that blockchain
    apply_blockchain_defaults(args)
