...
def main(
    ...
    tokens["address"] = tokens["address"].astype(str)
    tokens["key"] = tokens["key"].astype(str)
    tokens["symbol"] = tokens["symbol"].astype(str)
    ...
    mgr.carbon_inititalized = False
    mgr.pool_data = [
        pool for pool in mgr.pool_data if pool["exchange_name"] != "carbon_v1"
    ]
    for idx, pool in enumerate(mgr.pool_data):
        pool["last_updated_block"] = 0
...
def run(
    ...
    for p in mgr.pool_data:
        assert "last_updated_block" in p.keys()
    while True:
        # try:
